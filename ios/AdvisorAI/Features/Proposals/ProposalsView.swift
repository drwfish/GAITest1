...
struct ProposalsView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            if selectedSymbol == nil {
                selectedSymbol = sortedSymbols.first
            }
        }
    }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Advisory plane — typed candidates for human review.")
                ...
                HStack(alignment: .center, spacing: 12) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(acct.name)
                            ...
                            .foregroundStyle(Theme.Color.textPrimary)
                        Text("\(acct.id) · \(acct.mandate.strategy)")
                            .font(Theme.Font.mono(11))
                            .foregroundStyle(Theme.Color.textSecondary)
                    }
                    Spacer()
                    VStack(alignment: .trailing, spacing: 2) {
                        Text(Fmt.usd(acct.aum))
    ...
    private var sortedSymbols: [String] {
        state.market.snapshots.keys.sorted()
    }
        ...
        GlassCard {
            VStack(alignment: .leading, spacing: 14) {
                ...
                NeonButton(title: generating ? "Generating…" : "Generate Proposal",
                           ...
                           tone: .accent,
                           fill: true,
                           disabled: selectedSymbol == nil || generating) {
                    generate()
                }
            }
        }
    ...
    private func symbolPill(_ sym: String) -> some View {
        ...
        return Button {
            ...
            .background(
                Capsule().fill(selected ? Theme.Color.accent : Theme.Color.surfaceHi)
            )
            .overlay(
                Capsule().strokeBorder(selected ? Color.clear : Theme.Color.surfaceStroke, lineWidth: 0.8)
            )
    ...
    private func generate() {
        ...
        generating = true
        // small delay to make the UX feel like work happened
    ...
    private var proposalsList: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if state.proposals.isEmpty {
                ...
                VStack(spacing: 14) {
                    ForEach(Array(state.proposals.reversed()), id: \.id) { proposal in
        ...
        GlassCard {
            VStack(spacing: 10) {
                ...
                Text("No proposals yet.")
                    .font(Theme.Font.display(15, weight: .semibold))
                    .foregroundStyle(Theme.Color.textPrimary)
                Text("Generate one to see the pipeline come alive.")
...
private struct ProposalCard: View {
    ...
    private var metricsRow: some View {
        HStack(spacing: 12) {
            metric(label: "QTY", value: Fmt.qty(proposal.intent.qty))
            metric(label: "LIMIT",
                   value: proposal.intent.limitPrice.map { Fmt.usd($0, decimals: 2) } ?? "MKT")
            metric(label: "ALPHA",
                   value: String(format: "%+.0f bps", proposal.expectedAlphaBps),
                   color: proposal.expectedAlphaBps > 0 ? Theme.Color.success : Theme.Color.warn)
        }
    }

    private func metric(label: String, value: String, color: Color = Theme.Color.textPrimary) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            Text(label)
                .font(Theme.Font.mono(9, weight: .bold))
                .tracking(1.4)
        ...
        .overlay(RoundedRectangle(cornerRadius: 10, style: .continuous)
            .strokeBorder(Theme.Color.surfaceStroke, lineWidth: 0.7))
    }
        ...
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 6) {
                Text(proposal.rationale.headline)
                    ...
                    .foregroundStyle(Theme.Color.textPrimary)
                    .lineLimit(2)
                Text(relativeTime(proposal.generatedAt))
